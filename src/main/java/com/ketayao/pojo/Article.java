...
public class Article extends POJO implements Searchable {
    ...
    private static final String ARTICLEIDS_ADMIN  = "categoryId = ? AND trash = 0 ORDER BY topTime DESC, id DESC";
    private static final String ARTICLEIDS2       = "userId = ? AND trash = ? ORDER BY topTime DESC, id DESC";
    private static final String ARTICLEIDS2_ADMIN = "trash = ? ORDER BY topTime DESC, id DESC";
    private static final String HOME_ARTICLEIDS   = "trash = 0 AND status = '"
                                                    + Article.Status.PUBLISH
                                                    + "' ORDER BY topTime DESC, id DESC";
    private static final String FIND_NEWEST       = "trash = 0 AND status = '"
                                                    + Article.Status.PUBLISH + "' ORDER BY id DESC";
    ...
    public List<Article> findPage(Long userId, long categoryId, PageInfo pageInfo) {
        List<Long> ids = null;
        if (userId == null) {
            ids = getIds(ARTICLEIDS_ADMIN, categoryId);
        } else {
            ids = getIds(ARTICLEIDS, userId, categoryId);
        }
    ...
    public List<Article> findPage(Long userId, Boolean trash, PageInfo pageInfo) {
        List<Long> ids = null;
        if (userId == null) {
            ids = getIds(ARTICLEIDS2_ADMIN, BooleanUtils.toInteger(trash));
        } else {
            ids = getIds(ARTICLEIDS2, userId, BooleanUtils.toInteger(trash));
        }
    ...
    public List<Article> findTitleImages(PageInfo pageInfo) {
        List<Long> ids = getIds("imgUrl is not null AND trash = 0 AND status = '"
                                + Article.Status.PUBLISH + "' ORDER BY id DESC");
        pageInfo.setTotalRec(ids.size());
        List<Long> returnIds = ids.subList(pageInfo.getStartIndex(), pageInfo.getEndIndex());
    ...
    public List<Article> findExtendCategory(long categoryId, PageInfo pageInfo) {
        String sql = "categoryId = ? AND trash = 0 AND status = '" + Article.Status.PUBLISH
                     + "' ORDER BY topTime DESC, id DESC";
    ...
    public List<Article> findExtendPage(List<Long> categoryIds, PageInfo pageInfo) {
        ...
        String sql = "categoryId in (" + ids + ") AND trash = 0 AND status = '"
                     + Article.Status.PUBLISH + "' ORDER BY topTime DESC, id DESC";
     ...
     */
    public Article pre() {
        List<Long> ids = getIds(" id < ? AND trash = ? AND status = ? ORDER BY id DESC LIMIT 1",
