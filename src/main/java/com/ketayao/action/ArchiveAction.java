...
public class ArchiveAction extends AbstractAction {
    ...
    public String searchText(WebContext rc, String[] p) throws Exception {
        ...
        List<Long> ids = SystemInitServlet.indexHolder.find(Article.class, query, null,
            new Sort(new SortField("postTime", SortField.Type.LONG, true)), pageInfo.getPageIndex(),
            pageInfo.getPageSize());

        List<Article> articles = Article.INSTANCE.loadList(ids);
    ...
    public String month(WebContext rc, String[] p) throws Exception {
        ...
        List<Article> articles = Article.INSTANCE.findByMonth(new Timestamp(date.getTime()),
            pageInfo);
