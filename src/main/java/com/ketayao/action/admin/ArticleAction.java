...
public class ArticleAction extends AbstractAction {
    ...
    public String create(WebContext rc) throws IllegalAccessException, InvocationTargetException {
        ...
        if (StringUtils.isBlank(article.getImgUrl())) {
    ...
    public String read(WebContext rc) {
        ...
        boolean adminRole = isAdmin(user);

        if (categoryId > 0) {
            articles = Article.INSTANCE.findPage(adminRole ? null : user.getId(), categoryId,
                pageInfo);
            pageInfo.setUrl("read?categoryId=" + categoryId + "&pageIndex=");
        } else {
            articles = Article.INSTANCE.findPage(adminRole ? null : user.getId(), false, pageInfo);
    ...
    public String postUpdate(WebContext rc) throws Exception {
        ...
        User user = User.getLoginUser(rc);
        if (!isAdmin(user) && article.getUserId() != user.getId()) {
            throw new PermissionException("user deny");
        }
