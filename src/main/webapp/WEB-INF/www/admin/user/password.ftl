...
<div id="main">
	<form action="${rc.contextPath}/admin/user/modifyPwd" class="jNice" method="POST" id="formID">
		...
		<fieldset>
			<p>
				<label>${bundle("user.password.old")}<font color="red">*</font>:
				</label><input type="password" name="oldPassword" class="validate[required,minSize[5],maxSize[12]] text-long" id="oldPassword"/><#if notsame??><span class="lovej-validate"><@s.message code="validate.password.notsame"/></span></#if>
			</p>
			<p>
